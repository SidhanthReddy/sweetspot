...
import { cakeData } from '../data/cake_data.js'
      ...
      <div className="flex">
        ...
        <div className='flex space-x-16 animate-loop-scroll'>
          {duplicatedCakes.map((cake, index) => (
            <div key={`carousel1-${cake.id}-${index}`} className="w-[320px] flex-shrink-0 bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300">
              {/* Image Section */}
              ...
              <div className="p-4 space-y-3">
                ...
                <IngredientsToolTip 
                  cake={cake} 
                  cardId={`carousel1-${cake.id}-${index}`} 
                />
        ...
        <div className='flex space-x-16 animate-loop-scroll ml-16'>
          {duplicatedCakes.map((cake, index) => (
            <div key={`carousel2-${cake.id}-${index}`} className="w-[320px] flex-shrink-0 bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300">
              {/* Image Section */}
              <div className="w-full h-[200px] overflow-hidden rounded-t-lg">
                <img 
                  src={cake.image} 
              ...
              <div className="p-4 space-y-3">
                ...
                <IngredientsToolTip 
                  cake={cake} 
                  cardId={`carousel2-${cake.id}-${index}`} 
                />
